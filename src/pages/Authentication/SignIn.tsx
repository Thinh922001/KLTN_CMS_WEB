...
const SignIn = () => {
  const [userInfo, setUserInfo] = React.useState<ILogin>({
    email: '',
    password: '',
  });
  ...
  const handleLogin = () => {
    handleStateApi(async () => {
      const resLogin: IResponseApi = await apiLogin(userInfo);
      if (resLogin.statusCode != 200) {
        if (Array.isArray(resLogin.message)) {
          resLogin.message.forEach((item) => {
            toastMessage(item, 'error');
          });
        } else {
          toastMessage(resLogin.message, 'error');
        }
      } else {
        toastMessage('Đăng nhập thành công', 'success');
        const dataInfo: IAuth = resLogin.data;
        localStorage.setItem('accessToken', dataInfo.auth.accessToken);
        localStorage.setItem('refreshToken', dataInfo.auth.refreshToken);
        localStorage.setItem('expired', dataInfo.auth.expired);
        localStorage.setItem('tokenType', dataInfo.auth.tokenType);
        localStorage.setItem('admin', JSON.stringify(dataInfo.admin));
        navigate('/');
      }
    });
  };
        ...
        <div className="flex flex-wrap items-center">
          <div className="hidden w-full xl:block xl:w-1/2">
            <div className="py-17.5 px-26 text-center">
              <p className="2xl:px-20">Trang chủ quản lý sản phẩm</p>
          ...
          >
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              <h2 className="mb-9 text-2xl font-bold text-black dark:text-white sm:text-title-xl2 text-center">
                Đăng nhập vào hệ thống quản lý sản phẩm
              </h2>
              ...
              <div>
                ...
                <div className="mb-6">
                  ...
                  <div className="relative">
                    ...
                    <span
                      className="absolute right-4 top-4 text-xl text-black dark:text-white cursor-pointer"
                      onMouseDown={() => setShowPassword(true)}
                      onMouseUp={() => setShowPassword(false)}
                      onMouseLeave={() => setShowPassword(false)}
                    >
                      {userInfo.password.length > 0 ? <Eye /> : <Lock />}
                    </span>
                  </div>
                </div>
